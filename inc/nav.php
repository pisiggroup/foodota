<?php
if (!function_exists('foodota_mycustom_menu')) {
    function foodota_mycustom_menu($item_id, $item, $menu_level = 1, $show_indicator = true)
    {
        $item_data = array();
        $arr = array();
        $angle = $angleAerrow = '';
...
if (!function_exists('foodota_main_menu')) {
    function foodota_main_menu($theme_location)
    {
        $menu_html = '';
            ...
            if (isset($menu->term_id)) {
                ...
                foreach ($menu_items as $item) {
                    if ($item->menu_item_parent == 0) {
                        $level1 = 1;
                        $menuItems1 = foodota_mycustom_menu($item->ID, $item, 1);
                        $openSide = '';
                        ...
                        foreach ($menu_items as $sub_item) {
                            if ($item->ID == $sub_item->menu_item_parent) {
                                ...
                                if ($mainMenuClass == 'drop-down' || $mainMenuClass == 'drop-down-multilevel') {
                                    $menuItems2 = foodota_mycustom_menu($sub_item->ID, $sub_item, 2);
                                    if ($level1 == 1) {
                                    ...
                                    foreach ($menu_items as $sub_sub_item) {
                                        if ($sub_item->ID == $sub_sub_item->menu_item_parent) {
                                            ...
                                            $menuItems3 = foodota_mycustom_menu($sub_sub_item->ID, $sub_sub_item, 3, $show_indicator);
                                            $mega_menu_html .= '<li>';
                ...
                $menu_html .= '<li><a href="' . esc_url(home_url('/')) . '">' . esc_html__("Home", "foodota") . '</a></li>';
            }
        } else {
            $menu_html .= '<li><a href="' . esc_url(home_url('/')) . '">' . esc_html__("Home", "foodota") . '</a></li>';
        }
        echo foodota_navcustom_callback($menu_html, $theme_location);
    }
}

add_action('init', array('foodota_Nav_Menu_Item_Custom_Fields', 'setup'));
if (!class_exists('foodota_Nav_Menu_Item_Custom_Fields')) {
    class foodota_Nav_Menu_Item_Custom_Fields
    {
        ...
        static function setup()
        {
            if (!is_admin())
                return;
            $new_fields = apply_filters('foodota_menu_item_additional_fields', array());
            if (empty($new_fields))
                return;

            self::$options['fields'] = self::get_fields_schema($new_fields);
            add_filter('wp_edit_nav_menu_walker', function () {
                return 'foodota_Walker_Nav_Menu_Edit';
            });
            ...
            foreach (self::$options['fields'] as $field) {
                ...
                if ($field['input_type'] == 'only_text') {
                    ...
                    if ($field['name'] == 'is_megamenu_class') {
                        ...
                        $vals = array(esc_html__("Select Option", "foodota") => "", esc_html__("Simple Menu", "foodota") => "simple-menu", esc_html__("Mega Menu", "foodota") => "mega-menu");
                    ...
                    if ($field['name'] == 'is_megamenu') {
                        ...
                        for ($size = 12; $size >= 1; $size--) {
                            ...
                            $ar .= '<option value="col-12 col-sm-12 col-md-' . esc_attr($size) . ' col-lg-' . esc_attr($size) . '" ' . $selected . '>' . esc_html__("Column size ", "foodota") . $size . '</option>';
                        }
                    }
                    /* Main menu column */
                    if ($field['name'] == 'menu_open_side') {
                        $savedValue = get_post_meta($item->ID, self::get_menu_item_postmeta_key($field['name']), true);
                        $vals = array(esc_html__("Select Option", "foodota") => "", esc_html__("Left to Right", "foodota") => "submenu-left", esc_html__("Right to Left", "foodota") => "submenu-right");
                        $ar = '';
                        foreach ($vals as $val => $name) {
                            $selected = ($savedValue == $name) ? 'selected="selected"' : '';
                            $ar .= '<option value="' . esc_attr($name) . '" ' . $selected . '>' . esc_html($val) . '</option>';
                    ...
                    if ($field['name'] == 'menu_simple_offset') {
                        $savedValue1 = get_post_meta($item->ID, self::get_menu_item_postmeta_key($field['name']), true);
                        $ar = '';
                        $ar .= '<option value="">' . esc_html__("Select Option - (No Offset)", "foodota") . '</option>';
                        for ($size = 12; $size >= 1; $size--) {
                            $selected = ('offset-' . $size == $savedValue1) ? 'selected="selected"' : '';
                            $ar .= '<option value="offset-' . esc_attr($size) . '" ' . $selected . '>' . esc_html__("Column Offset ", "foodota") . $size . '</option>';
                        }
                    }
                    ...
                    if ($field['name'] == 'menu_vertical_offset') {
                        ...
                        for ($size = 12; $size >= 1; $size--) {
                            ...
                            $ar .= '<option value="offset-' . esc_attr($size) . '-vertical" ' . $selected . '>' . esc_html__("Vertical Offset", "foodota") . $size . '</option>';
                        }
                    }
                    $field['value'] = $ar;
                    $new_fields .= str_replace(
...
if (!class_exists('foodota_Walker_Nav_Menu_Edit')) {
    class foodota_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit
    {
        function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
        {
            $item_output = '';
            parent::start_el($item_output, $item, $depth, $args);
            if ($new_fields = foodota_Nav_Menu_Item_Custom_Fields::get_field($item, $depth, $args)) {
...
// Somewhere in config...
add_filter('foodota_menu_item_additional_fields', 'foodota_item_additional_fields');
if (!function_exists('foodota_item_additional_fields')) {
    function foodota_item_additional_fields($fields)
    {
        $fields['some_text'] = array(
            'name' => 'some_text',
            'label' => '*' . esc_html__('If you want to hide column name in mega menu just write {HideMe} in  Navigation Label', 'foodota'),
            'input_type' => 'only_text',
        );
        $fields['color'] = array(
            'name' => 'color',
            'label' => esc_html__('Lebel Color', 'foodota'),
            'container_class' => 'link-color',
            'input_type' => 'color',
        );
        $fields['color_text'] = array(
            'name' => 'color_text',
            'label' => esc_html__('Lebel Text', 'foodota'),
            'container_class' => 'link-color-text',
            'placeholder' => esc_html__('Enter some text', 'foodota'),
            'input_type' => 'text',
        );
        $fields['megamenu_class'] = array(
            'name' => 'is_megamenu_class',
            'label' => esc_html__('This select option is only for 1st level menu', 'foodota'),
            'container_class' => 'megamenu-classes',
            'input_type' => 'select',
        );
        $fields['megamenu'] = array(
            'name' => 'is_megamenu',
            'label' => esc_html__('In parent add bigger value and in 2nd level add small value', 'foodota'),
            'container_class' => 'megamenu-checkbox',
            'input_type' => 'select',
        );
        $fields['menu_icons'] = array(
            'name' => 'menu_icons',
            'label' => esc_html__('Menu Icons (Font Awesome Icons). ', 'foodota') . 'http://fontawesome.io/icons/',
        ...
        $fields['menu_open_side'] = array(
            'name' => 'menu_open_side',
            'label' => esc_html__('Select Options - For simple menu only', 'foodota'),
...
if (!function_exists('foodota_navcustom_callback')) {
    function foodota_navcustom_callback($menu_html = '', $theme_location = '')
    {
        return $menu_html;
    }
}